...
void TestPerceptronReal(std::size_t train_set_size,
                        std::uniform_real_distribution<T> dis1,
                        std::uniform_real_distribution<T> dis2) {
  unialgo::Perceptron p = unialgo::Perceptron(3);
  p.PrintWeights();

  T *train_set = static_cast<T *>(
      unialgo::utils::aligned_alloc(sizeof(T), sizeof(T) * 4 * train_set_size));
  ...
  T *test =
      static_cast<T *>(unialgo::utils::aligned_alloc(sizeof(T), 3 * sizeof(T)));

  auto testVector =
      [](T *testVec, std::uniform_real_distribution<T> &distribution,
         std::mt19937 &device, unialgo::Perceptron &p, bool target) {
        ...
        std::cout << "result of test using vector (" << testVec[0] << ", "
                  << testVec[1] << ", " << testVec[2] << ") is : " << res
...
template <typename T>
void TestPerceptron(std::size_t train_set_size,
                    std::uniform_int_distribution<T> dis1,
                    std::uniform_int_distribution<T> dis2) {
  unialgo::Perceptron p = unialgo::Perceptron(3);
  p.PrintWeights();
  ...
  auto testVector =
      [](T *testVec, std::uniform_int_distribution<T> &distribution,
         std::mt19937 &device, unialgo::Perceptron &p, bool target) {
        testVec[0] = static_cast<T>(distribution(device));
        testVec[1] = static_cast<T>(distribution(device));
        testVec[2] = static_cast<T>(distribution(device));
...
int main() {
  TestPerceptronReal<double>(600,
                             std::uniform_real_distribution<double>(0.0, 2.0),
