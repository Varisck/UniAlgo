 ...
 *
 *  Uses implementation of vectorized operation in dotProduct.h if
 * __UNIALGO_USE_AVX is defined.
 */
...
class Perceptron {
 public:
   ...
   * @param train_set_size Number of rows in training data Delta_w[i] =
   * mu(sum_{data in dataset}(t - y)(x[i]))
  ...
  bool Test(T *test_vec) {
    {
      ...
      T *test_vec_with_delta = static_cast<T *>(
          utils::aligned_alloc(sizeof(T), (input_size_ + 1) * sizeof(T)));
...
#include "UniAlgo/dotProduct/dotProduct.h"
 ...
 private:
  ...
  template <typename T>
  void inline FixErrorWithGD(T *train_set, double perceptronRes, T target,
                             double mu) {
